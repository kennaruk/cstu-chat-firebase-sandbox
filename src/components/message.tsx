...
import { Message as IMessage } from "../databases/types";
...
export const Message = ({ message }: Props) => {
  ...
  return (
    <div
      ...
      ${
        // message.username === session?.username ? "text-right" : 
        "text-left"
      }`
    }
    >
      ...
      >
        {(
          <div className="w-12 h-12 overflow-hidden flex-shrink-0 rounded">
            <a
              href={`https://github.com/${(message as any).content.username}`}
              target="_blank"
            ...
            >
              <img
                width={50}
                height={50}
                src={(message as any)?.content?.img}
              />
            </a>
          </div>
        )}
        ...
        >
          {
          // message.username !== session?.username && (
            <span className="font-bold">{(message as any).content.name}:&nbsp;</span>
          // )
          }
          <span className="max-w-sm">{(message as any).content.quote}</span>
        </span>
      </div>
